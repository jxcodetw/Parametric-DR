...
class UMAP_NN():
	def __init__(self, device, network, n_epochs, batch_size=4096, neg_rate=5):
		self.device = device
	...
	def fit(self, data, callback):
		...
		print('calc V')
		V_csc = fuzzy_simplicial_set(data, n_neighbors=15, random_state=np.random.RandomState(42), metric='euclidean')
